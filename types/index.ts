...
export type TaskState = 'gotta-start' | 'in-progress' | 'nearly-done' | 'paused' | 'completed' | string;
export type SubtaskState = 'todo' | 'in-progress' | 'completed' | string;
...
export interface CalendarEvent {
  ...
  endTime?: string;
  duration?: number;
  type: 'task-deadline' | 'subtask-scheduled' | 'personal-event' | 'meeting' | 'event' | 'task' | 'idea' | 'blocked';
...
export interface AppSettings {
  ...
  subtaskKanbanColumns?: KanbanColumn[]; // Columns for subtask kanban in task detail
  domainOrder: string[]; // Array of domain IDs in order
}
